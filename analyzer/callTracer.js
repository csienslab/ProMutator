...

{
    ...
    makeLog: function(log, logSize) {
        var topics = [];
        var mStart = log.stack.peek(0).valueOf();
        var mSize = log.stack.peek(1).valueOf();
        for (var i = 0; i < logSize; i++) {
            var addr = log.stack.peek(2 + i).toString(16);
            if (addr.length < 64) {
                addr = '0'.repeat(64 - addr.length) + addr
            } 
            topics.push('0x' + addr.toString(16));
        }
        this.events.push({
            address: toHex(log.contract.getAddress()),
            data: toHex(log.memory.slice(mStart, mStart+mSize)),
            topics: topics,
